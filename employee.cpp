...
{
    QSqlQuery query;
    // Utiliser le nom de colonne réel : MOT_DE_PASSE
    query.prepare("UPDATE EMPLOYE SET MOT_DE_PASSE = :motdepass WHERE EMAIL = :email");
    query.bindValue(":motdepass", nouveauMotDePasse);
    query.bindValue(":email", email);
