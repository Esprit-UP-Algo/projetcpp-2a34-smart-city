...
void LoginWindow::configurerInterface()
{
    ...
    QPixmap logoPixmap(":/resources.qrc/logo.png");  // ✅ Utilise le chemin correct

    qDebug() << "Chargement du logo:" << (!logoPixmap.isNull() ? "Succès" : "Échec");
...
bool LoginWindow::validerConnexion(const QString& email, const QString& motDePasse)
{
    ...
    QSqlQuery query;
    query.prepare("SELECT CIN, NOM, PRENOM, MOTDEPASS FROM EMPLOYE WHERE EMAIL = :email");
