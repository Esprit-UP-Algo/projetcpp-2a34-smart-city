...
QSqlQueryModel* Dechet::rechercher(QString critere, QString valeur)
{
    ...
    QString req;
    if (critere == "CIN") {
        req = "SELECT CIN, DATE_COLLECTE, LIEU_COLLECTE, VOLUME_POIDS, ETAT FROM DECHETS WHERE TO_CHAR(CIN) LIKE :valeur";
    } else if (critere == "LIEU_COLLECTE") {
        req = "SELECT CIN, DATE_COLLECTE, LIEU_COLLECTE, VOLUME_POIDS, ETAT FROM DECHETS WHERE LIEU_COLLECTE LIKE :valeur";
    } else {
